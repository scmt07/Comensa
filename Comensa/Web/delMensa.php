  ...
  if($_POST['action'] == 'login') {
    ...
    if (empty($_POST["user"])) {
      ...
      if($aux < 1) {
        ...
        else if ($saldo['saldo'] != 0.00) {
          $userErr = "*Usuário ainda tem saldo, quite o saldo primeiro;";
          $vazio = True;
        }
      }
    }
    #cadastra o usuario
    if($vazio == False) {
      $sql_query = "DELETE FROM contas WHERE idMensa = '$row[idMensa]' AND idEstab = '$_SESSION[id]';";
  ...
  <div class="item">
    <div class="content">
      <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        <label for="Cadastro">Deletar Conta</label><br>
        ...
        <div class="input-group">
          <button type="submit" onclick="return confirm('Deletar conta?');" name="action" value="login" class="btn btn-primary">Deletar</button>
        </div>
