...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if($_POST['action'] == 'login') {
    ...
    if (empty($_POST["nome"])) {
      ...
      if($aux < 1) {
        $nomeErr = "*Produto não existe";
        $vazio = True;
      }
    }
    #deleta o produto
    if($vazio == False) {
      $sql_query = "DELETE FROM produto WHERE idProd = '$row[idProd]' AND idEstab = '$_SESSION[id]';";
  ...
  <div class="item">
    <div class="content">
      <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        <label for="Cadastro">Deletar Produto</label><br>
        ...
        <div class="input-group">
          <button type="submit" onclick="return confirm('Deletar produto?');" name="action" value="login" class="btn btn-primary">Deletar</button>
        </div>
